...
* **极致视觉体验**：后期光晕 (Bloom) 与辉光，营造梦幻氛围
* **记忆画廊**：拍立得风格悬浮照片，双面渲染，底部随机艺术英文字并带写实阴影
* **AI 手势控制**（桌面端）：无需鼠标，通过摄像头完成树形态、视角与相片交互
* **触摸手势**（移动端）：滑动旋转、双击切换、双指缩放等完整触控支持
* **双手协作**（桌面端）：第一只手捏合拖移，第二只手捏合后用两手食指距离控制选中照片缩放
* **响应式设计**：自动适配桌面端和移动端，针对性能优化
* **可定制**：支持替换照片、调整数量、切换圣诞元素风格
...
### 5. 部署到生产环境 🌐
本项目已配置好 Cloudflare Pages 部署，支持一键部署：
...
> 💡 **提示**: 移动端为优化性能和用户体验，禁用了相机手势识别，改用更直观的触摸操作。
